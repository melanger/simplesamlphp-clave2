...
class sspmod_clave_Auth_Source_SP extends SimpleSAML\Auth\Source
{
    ...
    public function __construct($info, $config)
    {
        ...
        SimpleSAML\Logger::debug(
            'eIDAS IDP remote metadata (' . $idpEntityId . '): ' . print_r($this->idpMetadata, true)
        );
    ...
    public function startSSO(string $idp, array $state)
    {
        ...
        if ($this->dialect === 'stork') {
            ...
            if (! $useMetadataUrl) {
                $reqIssuer = $this->spMetadata->getString(
                    'issuer',
        ...
        if ($this->dialect === 'eidas') { //On eIDAS, we always get the country selector value.
            ...
            if (isset($state['eidas:requestData']['LoA'])
                ...
                $LoA = $this->spMetadata->getString(
                    'LoA',
                    $remoteSpMeta->getString('LoA', sspmod_clave_SPlib::LOA_LOW)
                );
                SimpleSAML\Logger::debug('Setting LoA from Metadata: ' . $LoA);
            }
            $QAA = sspmod_clave_SPlib::loaToQaa($LoA);
            $state['eidas:requestData']['QAA'] = sspmod_clave_SPlib::loaToQaa(
                $LoA
            ); //We overwrite it to avoid it overwriting the LoA later when the remote SP spoke stork
            ...
            foreach ($state['eidas:requestData']['requestedAttributes'] as $attr) {
                ...
                if ($this->dialect === 'eidas') {//el dialecto del SP hosted
                    ...
                    if (array_key_exists('friendlyName', $attr)) {
                        ...
                        $name = sspmod_clave_SPlib::getEidasFriendlyName(
                            $attr['name']
                        ); //We are expecting eIDAS attribute full names, so 1
                        if ($name === '') {
                            $name = $attr['name'];
                        }
                    }
    ...
    public function handleResponse(array $state, string $idp, array $attributes)
    {
        ...
        $authProcState = [
            'saml:sp:IdP' => $idp,
            'saml:sp:State' => $state,
            'ReturnCall' => ['sspmod_clave_Auth_Source_SP', 'onProcessingCompleted'],

            'Attributes' => $attributes,
            //Add received attributes to the state that will, in the end, be returned to the IdP
    ...
    private function startDisco(array $state): bool
    {
        ...
        $returnTo = SimpleSAML\Module::getModuleURL('clave/sp/discoresp.php', [
            'AuthID' => $id,
        ...
        $params = [
            ...
            'return' => $returnTo,
            //The script to go on with the auth process (contains the authsource ID)
    ...
    private function redirect($destination, $req, $state)
    {
      ...
      //Get the remote SP metadata
        ...
        if ($this->subdialect === 'clave-2.0') {
            ...
            } //A value is always required to ensure eIDAS node response includes a relay state and is accepted by Clave2
